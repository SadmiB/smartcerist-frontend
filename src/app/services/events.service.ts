...
export class EventsService {
  ...
  getAllEvents(){
    return this.httpClient.get<EventObj[]>(Consts.BASE_URL + `/events`)
    .subscribe(res => {
      this.eventStore=res;
  ...
  addEvent(_event){
    console.log("---------------------------------------------------------")
    return this.httpClient.post<EventObj>(Consts.BASE_URL+`/events`,_event)
    .subscribe(res => {
      this.socket.emit("add-room",_event.homeId)
      console.log(res)
      this.getAllEvents();
    },error =>{
  ...
  getConnectedUserRoomsIds (tokenHeader,homeId) {
    return this.httpClient.get<String[]>(Consts.BASE_URL + `/user/${homeId}/roomsIds`,{headers:tokenHeader})
    .subscribe(res => {
      res.forEach(roomId => {
